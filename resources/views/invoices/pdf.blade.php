...
@php
  ...
  $showTaxPercentLabel = (bool) ($companyAttrs['show_tax_percent_on_pdf'] ?? true)
    && (bool) ($invoiceAttrs['show_tax_percent_label'] ?? true);
  $customerRefType = strtolower((string) ($invoice->salesOrder?->customer_ref_type ?? 'po')) === 'spk' ? 'spk' : 'po';
...
<table class="totals" style="margin-top:16px;">
  ...
  @endif
  <tr>
    <td>PPN{{ $showTaxPercentLabel ? ' (' . rtrim(rtrim(number_format((float)$invoice->tax_percent, 2, '.', ''), '0'), '.') . '%)' : '' }}</td>
