                ...
                <div class="row g-3">
                  ...
                  <div class="col-md-6">
                    ...
                    <select name="jenis_id" class="form-select" required>
                      ...
                      @foreach ($jenisList as $j)
                        <option value="{{ $j->id }}" @selected(old('jenis_id', $customer->jenis_id) == $j->id)>
                          {{ $j->name }}
                        </option>
                      @endforeach
                    </select>
                  ...
                  <div class="col-md-6">
                    ...
                    @if(auth()->user()->hasAnyRole(['Admin','SuperAdmin','Finance']))
                      <select name="sales_user_id" class="form-select" required>
                        ...
                        @foreach($salesUsers as $u)
                          <option value="{{ $u->id }}"
                            @selected(old('sales_user_id', $customer->sales_user_id) == $u->id)>
                            {{ $u->name }}
                          </option>
                        @endforeach
                      </select>
                    @else
                      ...
                      <input type="hidden" name="sales_user_id" value="{{ $customer->sales_user_id ?: auth()->id() }}">
                    @endif
                  </div>
                  ...
                  <div class="col-md-6">
                    ...
                    <input type="text" name="website" value="{{ old('website', $customer->website) }}" class="form-control" placeholder="https://... (optional)">
                  </div>
              ...
              <div class="tab-pane" id="tab-billing">
                <div class="row g-3">
                  ...
                  <div class="col-12">
                    ...
                    <textarea name="billing_street" class="form-control" rows="2">{{ old('billing_street', $customer->billing_street) }}</textarea>
                  </div>

                  <div class="col-md-4">
                    <label class="form-label">City</label>
                    <input name="billing_city" class="form-control" value="{{ old('billing_city', $customer->billing_city) }}">
                  </div>

                  <div class="col-md-4">
                    <label class="form-label">State/Province</label>
                    <input name="billing_state" class="form-control" value="{{ old('billing_state', $customer->billing_state) }}">
                  </div>

                  <div class="col-md-4">
                    <label class="form-label">Zip</label>
                    <input name="billing_zip" class="form-control" value="{{ old('billing_zip', $customer->billing_zip) }}">
                  </div>

                  <div class="col-md-4">
                    <label class="form-label">Country (ISO2)</label>
                    <input name="billing_country" class="form-control" value="{{ old('billing_country', $customer->billing_country) }}" placeholder="ID">
                  </div>

                  <div class="col-12">
                    <label class="form-label">Notes</label>
                    <textarea name="billing_notes" class="form-control" rows="2">{{ old('billing_notes', $customer->billing_notes) }}</textarea>
                  </div>
                  ...
                  <div class="col-12">
                    <label class="form-label">Street</label>
                    <textarea name="shipping_street" class="form-control" rows="2">{{ old('shipping_street', $customer->shipping_street) }}</textarea>
                  </div>

                  <div class="col-md-4">
                    <label class="form-label">City</label>
                    <input name="shipping_city" class="form-control" value="{{ old('shipping_city', $customer->shipping_city) }}">
                  </div>

                  <div class="col-md-4">
                    <label class="form-label">State/Province</label>
                    <input name="shipping_state" class="form-control" value="{{ old('shipping_state', $customer->shipping_state) }}">
                  </div>

                  <div class="col-md-4">
                    <label class="form-label">Zip</label>
                    <input name="shipping_zip" class="form-control" value="{{ old('shipping_zip', $customer->shipping_zip) }}">
                  </div>

                  <div class="col-md-4">
                    <label class="form-label">Country (ISO2)</label>
                    <input name="shipping_country" class="form-control" value="{{ old('shipping_country', $customer->shipping_country) }}" placeholder="ID">
                  </div>

                  <div class="col-12">
                    <label class="form-label">Notes</label>
                    <textarea name="shipping_notes" class="form-control" rows="2">{{ old('shipping_notes', $customer->shipping_notes) }}</textarea>
                  </div>
