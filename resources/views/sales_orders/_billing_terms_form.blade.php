...
(function () {
  ...
  window.setBillingTermsRows = function(rows) {
    ...
    (rows || []).forEach((row, idx) => {
      ...
      if (!tr) return;
      const topCodeInput = tr.querySelector('select[name*="[top_code]"]');
      if (topCodeInput) topCodeInput.value = row.top_code || '';
      const percentInput = tr.querySelector('input[name*="[percent]"]');
      if (percentInput) percentInput.value = row.percent ?? 0;
      const triggerInput = tr.querySelector('select[name*="[due_trigger]"]');
      if (triggerInput) triggerInput.value = row.due_trigger || '';
      const offsetInput = tr.querySelector('input[name*="[offset_days]"]');
      if (offsetInput) offsetInput.value = row.offset_days ?? '';
      const dayInput = tr.querySelector('input[name*="[day_of_month]"]');
      if (dayInput) dayInput.value = row.day_of_month ?? '';
      const noteInput = tr.querySelector('input[name*="[note]"]');
