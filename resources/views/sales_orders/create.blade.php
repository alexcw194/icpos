  ...
  <form action="{{ route('sales-orders.store') }}" method="POST" class="card" id="soForm">
    ...
    <div class="card-body">
      <div class="row g-3">
        ...
        {{-- PO/SPK NUMBER --}}
        <div class="col-md-2">
          <label class="form-label" id="customer_ref_number_label">Customer PO Number</label>
...
<style>
  ...
  #linesTable .col-labor-unit { width: 10%; }
...
(function () {
  ...
  function syncCustomerRefLabels() {
    ...
    if (customerRefNumberLabel) {
      customerRefNumberLabel.textContent = isSpk ? 'Customer SPK Number' : 'Customer PO Number';
    }
  ...
  function addLineFromStage(){
    ...
    const isScope = ['project', 'maintenance'].includes(poTypeSelect?.value || '');
    const isProject = (poTypeSelect?.value || '') === 'project';
    setRowScopeState(tr, isScope, isProject);
    lineIdx++;
